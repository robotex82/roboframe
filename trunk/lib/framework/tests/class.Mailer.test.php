...
class TestOfMailerClass extends UnitTestCase {
  ...
  function setUp() {
    \Mailer\Base::set_view_root(realpath(dirname(__FILE__).'/../test_assets/Mailer/views'));
  }
  ...
  function test_load_settings_returns_array() {
    ...
    \Mailer\Base::set_configuration_file($filename);
    $this->assertTrue(is_array(\Mailer\Base::load_settings()), 'Load Settings should return an array');
  }
   
  function test_settings_array_includes_needed_keys() {
    $filename = dirname(__FILE__).'/../test_assets/Mailer/good_mailer.ini';
    \Mailer\Base::set_configuration_file($filename);
    $settings = \Mailer\Base::load_settings($filename);
  ...
  function test_loading_settings_throw_no_exception() {
    $filename = dirname(__FILE__).'/../test_assets/Mailer/good_mailer.ini';
    \Mailer\Base::set_configuration_file($filename);
    $m = \Mailer\Base::load_settings($filename);
